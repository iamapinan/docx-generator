...
### ตัวแปรแบบ Literal (ไม่แทนที่ค่า)

✨ **ฟีเจอร์ใหม่**: ใช้ `{!variable}` เมื่อต้องการให้แสดงเป็น `{variable}` ในเอกสารโดยไม่แทนที่ค่า

**ตัวอย่าง**:
- Template มี: `{!name}`, `{lastname}`
- ส่งข้อมูลมา: `{"lastname": "ใจดี"}`
- ผลลัพธ์:
  - `{!name}` → "{name}" (แสดงเป็น literal text)
  - `{lastname}` → "ใจดี" (แทนที่ค่าปกติ)

**การใช้งาน**:
- `{!variable}` - แสดงเป็น `{variable}` ในเอกสาร
- `{variable}` - แทนที่ด้วยค่าจริง หรือ `...........` หากไม่มีข้อมูล
...
**หมายเหตุ**: 
- ตัวแปรปกติ `{variable}` จะถูกแทนที่ด้วยค่าหรือจุด
- ตัวแปร literal `{!variable}` จะแสดงเป็น `{variable}` เสมอ ไม่แทนที่ค่า
...
2. **ตัวอย่าง Log Output**:
   ...
   🔒 Literal variables (no replacement): company, address
   ```
...
### v3.2.0 (Current)
- เพิ่มฟีเจอร์ Literal Variables: `{!variable}` แสดงเป็น `{variable}` โดยไม่แทนที่ค่า
- ปรับปรุง placeholder extraction ให้รองรับ literal variables
- เพิ่ม logging สำหรับ literal variables
- อัปเดต documentation สำหรับฟีเจอร์ใหม่
